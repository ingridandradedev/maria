...
# CONFIGURAÇÕES
LINK_REUNIAO_ORIGINAL = "https://teams.microsoft.com/l/meetup-join/19%3ameeting_ZWJkMWU1ZTEtN2Y2YS00ODM1LWI4MjctZDQ5NTIwMzRkNDc1%40thread.v2/0?context=%7b%22Tid%22%3a%22cbf42931-5cdd-43c3-97c9-47c2c1c4b0c3%22%2c%22Oid%22%3a%228e01e4d1-52b2-4870-be7a-34c1262ca17c%22%7d"
NOME_USUARIO = "GravadorBot"
DURACAO_MAXIMA = 60 * 60  # 1 hora
DISPOSITIVO_AUDIO = "Mixagem estéreo (Realtek(R) Audio)"  # Para áudio do sistema
...
# INICIA GRAVAÇÃO DE TELA + ÁUDIO COM FFMPEG
def iniciar_gravacao(nome_arquivo):
    nome_arquivo = nome_arquivo.replace(".mp3", ".mp4")
    comando = [
        r"C:\Users\Dell\Downloads\ffmpeg-7.1.1-essentials_build\ffmpeg-7.1.1-essentials_build\bin\ffmpeg.exe",
        "-y",
        "-f", "gdigrab",                     # Gravação da tela (Windows)
        "-framerate", "30",
        "-i", "desktop",
        "-f", "dshow",                       # Captura de áudio
        "-i", f"audio={DISPOSITIVO_AUDIO}",
        "-c:v", "libx264",
        "-preset", "ultrafast",
        "-c:a", "aac",
        "-b:a", "192k",
        "-af", "volume=2.5",                 # Aumenta o volume do áudio
        "-pix_fmt", "yuv420p",
        nome_arquivo
    ]
...
def entrar_e_gravar():
    ...
    with sync_playwright() as p:
        ...
        # Inicia gravação (tela + áudio)
        nome_arquivo = f"gravacao_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp4"
        processo_ffmpeg = iniciar_gravacao(nome_arquivo)
        print(f"🎥 Gravando tela e áudio em: {nome_arquivo}")
